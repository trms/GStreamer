    ...
    BMDDisplayMode mode, BMDPixelFormat format)
{
  ...
  switch (self->api_level) {
    ...
    case GST_DECKLINK2_API_LEVEL_11_4:
      hr = self->output_11_4->DoesSupportVideoMode (
    ...
    case GST_DECKLINK2_API_LEVEL_LATEST:
      hr = self->output->DoesSupportVideoMode (bmdVideoConnectionUnspecified,
          mode, format, bmdNoVideoOutputConversion, bmdSupportedVideoModeDefault,
...
gst_decklink2_output_new (IDeckLink * device, GstDeckLink2APILevel api_level)
{
  ...
  self = (GstDeckLink2Output *) g_object_new (GST_TYPE_DECKLINK2_OUTPUT,
      nullptr);
  ...
  HRESULT hr = E_FAIL;
   ...
   switch (self->api_level) {
    case GST_DECKLINK2_API_LEVEL_10_11:
      ...
      break;
    case GST_DECKLINK2_API_LEVEL_11_4:
      hr = self->output_11_4->StartScheduledPlayback (start_time,
          scale, speed);
      break;
    case GST_DECKLINK2_API_LEVEL_11_5_1:
    case GST_DECKLINK2_API_LEVEL_LATEST:
    ...
    IDeckLinkVideoFrame * frame, guint8 * audio_buf, gsize audio_buf_size)
{
  ...
  switch (self->api_level) {
    ...
    case GST_DECKLINK2_API_LEVEL_11_5_1:
    case GST_DECKLINK2_API_LEVEL_LATEST:
      hr = self->output->ScheduleVideoFrame (frame,
          self->pts, dur, GST_SECOND);
      break;
    default:
      g_assert_not_reached ();
  ...
  if (!self->prerolled) {
    ...
    if (audio_buf && audio_buf_size > 0) {
      ...
      hr = gst_decklink2_output_schedule_audio_samples (self,
            audio_buf, num_samples, 0, 0, nullptr);
      if (!gst_decklink2_result (hr)) {
    ...
    if (self->n_prerolled >= self->n_preroll_frames) {
      if (GST_AUDIO_INFO_IS_VALID (&self->audio_info)) {
        ...
        if (!gst_decklink2_result (hr)) {
          GST_ERROR_OBJECT (self, "Audio preroll failed, hr: 0x%x",
              (guint) hr);
          return hr;
        }
      }
...
gst_decklink2_output_stop_internal (GstDeckLink2Output * self)
{
  ...
  switch (self->api_level) {
    case GST_DECKLINK2_API_LEVEL_10_11:
      hr = self->output_10_11->StopScheduledPlayback (0, nullptr, 0);
      break;
    case GST_DECKLINK2_API_LEVEL_11_4:
      hr = self->output_11_4->StopScheduledPlayback (0, nullptr, 0);
      break;
    case GST_DECKLINK2_API_LEVEL_11_5_1:
    case GST_DECKLINK2_API_LEVEL_LATEST:
      hr = self->output->StopScheduledPlayback (0, nullptr, 0);
      break;
    default:
      g_assert_not_reached ();
      break;
  ...
  gst_decklink2_output_set_video_callback (self, nullptr);
...
public:
  ...
  /* IDeckLinkTimecode */
  BMDTimecodeBCD STDMETHODCALLTYPE
  GetBCD (void)
  {
...
public:
  ...
  HRESULT STDMETHODCALLTYPE
  SetTimecode (GstVideoTimeCode * timecode)
  {
...
private:
  ...
  IDeckLinkTimecode *timecode_ = nullptr;
  IDeckLinkVideoFrameAncillary *ancillary_ = nullptr;
};
    ...
    gint caption_line, gint afd_bar_line)
{
  ...
  if ((got_captions || afd_bar_line != 0)
    ...
    GstVideoAFDMeta *afd_meta = nullptr, *afd_meta2 = nullptr;
    GstVideoBarMeta *bar_meta = nullptr, *bar_meta2 = nullptr;
    GstMeta *meta;
    gpointer meta_iter;
    ...
    BMDAudioSampleType audio_sample_type, guint audio_channels)
{
  ...
  if (profile_id != bmdProfileDefault && output->profile_manager) {
    IDeckLinkProfile *profile = nullptr;
    hr = output->profile_manager->GetProfile (profile_id, &profile);
    if (gst_decklink2_result (hr)) {
    ...
    if (!gst_decklink2_result (hr))
      GST_WARNING_OBJECT (output, "Couldn't set profile");
  }
    ...
    if (gst_decklink2_result (hr) && supported) {
      ...
      if (mapping_format == GST_DECKLINK2_MAPPING_FORMAT_LEVEL_A) {
        if (output->config_10_11) {
          hr = output->config_10_11->SetFlag (
              bmdDeckLinkConfigSMPTELevelAOutput, true);
        } else if (output->config) {
          hr = output->config->SetFlag (
              bmdDeckLinkConfigSMPTELevelAOutput, true);
        }
      } else if (mapping_format == GST_DECKLINK2_MAPPING_FORMAT_LEVEL_B) {
        if (output->config_10_11) {
          hr = output->config_10_11->SetFlag (
              bmdDeckLinkConfigSMPTELevelAOutput, false);
        } else if (output->config) {
          hr = output->config->SetFlag (
              bmdDeckLinkConfigSMPTELevelAOutput, false);
        }
...
error:
  ...
  gst_decklink2_output_set_video_callback (output, nullptr);
  return hr;
}
