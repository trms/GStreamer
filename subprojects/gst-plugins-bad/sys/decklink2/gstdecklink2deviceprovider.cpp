...
gst_decklink2_device_create_element (GstDevice * device, const gchar * name)
{
  ...
  g_object_set (elem, "persistent-id", self->persistent_id, nullptr);
    ...
    const gchar * driver_ver, const gchar * api_ver)
{
  ...
  props = gst_structure_new ("properties",
      ...
      "persistent-id", G_TYPE_INT64, persistent_id, nullptr);

  if (max_audio_channels > 0) {
    gst_structure_set (props,
        "max-channels", G_TYPE_UINT, max_audio_channels, nullptr);
  }

  if (serial_number && serial_number[0] != '\0') {
    gst_structure_set (props,
        "serial-number", G_TYPE_STRING, serial_number, nullptr);
  }
