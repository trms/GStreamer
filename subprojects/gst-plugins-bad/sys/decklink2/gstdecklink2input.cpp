...
do_align_and_malloc(gsize size, gsize alignment)
{
  gsize offset = size % alignment;
  if (offset)
    size += (alignment - offset);

  return aligned_alloc (alignment, size);
}
#define ALIGNED_ALLOC(s,a) do_align_and_malloc(s,a)
...
public:
  ...
  HRESULT STDMETHODCALLTYPE
  AllocateBuffer (unsigned int size, void ** alloced)
  {
    ...
    if (!buf) {
      guint8 * new_buf = (guint8 *) ALIGNED_ALLOC (size + 128, 64);
      *((guint32 *) new_buf) = size;
      new_buf += 64;
      buf = new_buf;
    }
  ...
  ReleaseBuffer (void * buffer)
  {
    ...
    guint8 *aligned_buf = (guint8 *) buffer;
    guint32 size;

    aligned_buf -= 64;
    size = *((guint32 *) aligned_buf);
  ...
  switch (self->api_level) {
    case GST_DECKLINK2_API_LEVEL_10_11:
      hr = self->input_10_11->DoesSupportVideoMode (mode, format,
          bmdVideoOutputFlagDefault, &supported_v10_11, nullptr);
      if (hr != S_OK || supported_v10_11 != bmdDisplayModeSupported_v10_11)
        return FALSE;
      break;
    case GST_DECKLINK2_API_LEVEL_11_4:
      hr = self->input_11_4->DoesSupportVideoMode (bmdVideoConnectionUnspecified,
          mode, format, bmdSupportedVideoModeDefault, &supported);
      if (hr != S_OK || !supported)
        return FALSE;
      break;
    case GST_DECKLINK2_API_LEVEL_11_5_1:
      hr = self->input_11_5_1->DoesSupportVideoMode (
          bmdVideoConnectionUnspecified, mode, format,
...
gst_decklink2_input_new (IDeckLink * device, GstDeckLink2APILevel api_level)
{
  ...
  self = (GstDeckLink2Input *) g_object_new (GST_TYPE_DECKLINK2_INPUT,
      nullptr);
  ...
  switch (api_level) {
    ...
    case GST_DECKLINK2_API_LEVEL_11_5_1:
      ...
      break;
    case GST_DECKLINK2_API_LEVEL_LATEST:
      hr = device->QueryInterface (IID_IDeckLinkInput,
          (void **) &self->input);
      if (hr != S_OK) {
        gst_object_unref (self);
        return nullptr;
...
static HRESULT
gst_decklink2_input_set_callback (GstDeckLink2Input * self,
    IUnknown * callback)
{
  HRESULT hr = E_FAIL;
    ...
    BMDDetectedVideoInputFormatFlags flags)
{
  ...
  self->audio_offset = (guint64) -1;
  self->next_audio_offset = (guint64) -1;
...
{
  IDeckLinkVideoFrameAncillary *vanc_frame = nullptr;
  gint line;
  BMDPixelFormat pixel_format;
  GstVideoFormat video_format;
  gboolean found_cc = FALSE, found_afd_bar = FALSE;
    ...
    IDeckLinkVideoInputFrame * frame, IDeckLinkAudioInputPacket * packet)
{
  ...
  if (!clock) {
    GST_WARNING_OBJECT (self, "Frame arrived but we dont have configured clock");
  } else {
  ...
  if (frame) {
    ...
    if (gst_decklink2_result (hr)) {
      ...
      pts = gst_clock_adjust_with_calibration (nullptr, stream_time,
        self->current_time_mapping.xbase, self->current_time_mapping.b,
        self->current_time_mapping.num, self->current_time_mapping.den);
    ...
    if (hr == S_OK) {
      ...
      if (gst_decklink2_result (hr)) {
        ...
        gst_video_time_code_init (&tc, self->selected_mode.fps_n,
            self->selected_mode.fps_d, nullptr, tc_flags, h, m, s, f, 0);
  ...
  if (packet) {
    ...
    if (self->audio_offset == (guint64) -1 && !frame) {
    ...
    if (self->audio_offset == (guint64) -1) {
      ...
      audio_pts = gst_clock_adjust_with_calibration (nullptr,
          packet_time_in_gst,
        self->current_time_mapping.xbase, self->current_time_mapping.b,
        self->current_time_mapping.num, self->current_time_mapping.den);
    ...
    if (self->next_audio_offset == (guint64) -1) {
  ...
  if (buffer) {
    ...
    if (audio_size > 0) {
      ...
      GST_BUFFER_PTS (audio_buf) = gst_util_uint64_scale (
            self->audio_offset, GST_SECOND, self->audio_info.rate);
      GST_BUFFER_DURATION (audio_buf) = gst_util_uint64_scale (
            sample_count, GST_SECOND, self->audio_info.rate);
      ...
      sample = gst_sample_new (audio_buf, self->selected_audio_caps, nullptr,
          nullptr);
    ...
    sample = gst_sample_new (buffer, self->selected_video_caps, nullptr, nullptr);
...
gst_decklink2_input_stop_unlocked (GstDeckLink2Input * self)
{
  ...
  gst_decklink2_input_set_callback (self, nullptr);
   ...
   const GstDeckLink2InputAudioConfig * audio_config)
{
  ...
  input->audio_offset = (guint64) -1;
  input->next_audio_offset = (guint64) -1;
  ...
  if (audio_config->channels != GST_DECKLINK2_AUDIO_CHANNELS_DISABLED) {
    ...
    gst_audio_info_set_format (&input->audio_info,
        ...
        GST_AUDIO_FORMAT_S32LE : GST_AUDIO_FORMAT_S16LE, 48000, channels,
        nullptr);
